...
function exactTimeBandByDomain(domainKey) {
  ...
  if (["FOREIGN", "SETTLEMENT", "IMMIGRATION"].includes(domainKey)) {
    return "08:00-14:00";
  }
...
function exactTypeBoost(domainKey, event) {
  ...
  if (["FOREIGN", "SETTLEMENT"].includes(domainKey)) {
    if (["foreign", "entry", "settlement", "movement", "process", "relocation"].some((x) => type.includes(x))) boost += 4;
  }
...
function buildTimingSummary(master_timeline) {
  const valid = (master_timeline || []).filter(
    (e) => normalizeYear(e.date_marker) != null || e.month_or_phase != null
  );
  ...
  return {
    first_visible_year: normalizeYear(first?.date_marker) ?? null,
    first_visible_phase: first?.month_or_phase ?? null,
    first_visible_time_band: first?.time_band ?? null,

    last_visible_year: normalizeYear(last?.date_marker) ?? null,
...
function buildRecognitionTimeline(master_timeline) {
  return (master_timeline || [])
    .slice(0, 12)
    .map((e) => ({
      year: normalizeYear(e.date_marker) ?? null,
...
function buildProjectPasteBlock({
  ...
  return [
    ...
    "Past Phase Summary:",
    `BEFORE: ${phaseSummary.BEFORE?.visible_year ?? "UNKNOWN"} | ${phaseSummary.BEFORE?.visible_month_or_phase ?? "NO_PHASE"} | ${phaseSummary.BEFORE?.visible_time_band ?? "NO_TIME"} | ${phaseSummary.BEFORE?.visible_event_type ?? "NO_EVENT"}`,
    `PEAK: ${phaseSummary.PEAK?.visible_year ?? "UNKNOWN"} | ${phaseSummary.PEAK?.visible_month_or_phase ?? "NO_PHASE"} | ${phaseSummary.PEAK?.visible_time_band ?? "NO_TIME"} | ${phaseSummary.PEAK?.visible_event_type ?? "NO_EVENT"}`,
    `IMPACT: ${phaseSummary.IMPACT?.visible_year ?? "UNKNOWN"} | ${phaseSummary.IMPACT?.visible_month_or_phase ?? "NO_PHASE"} | ${phaseSummary.IMPACT?.visible_time_band ?? "NO_TIME"} | ${phaseSummary.IMPACT?.visible_event_type ?? "NO_EVENT"}`,
    `TRANSITION: ${phaseSummary.TRANSITION?.visible_year ?? "UNKNOWN"} | ${phaseSummary.TRANSITION?.visible_month_or_phase ?? "NO_PHASE"} | ${phaseSummary.TRANSITION?.visible_time_band ?? "NO_TIME"} | ${phaseSummary.TRANSITION?.visible_event_type ?? "NO_EVENT"}`,

    "Recognition Timeline:",
    ...event_summary.recognition_timeline.slice(0, 12).map(
      (e) =>
        `${e.year ?? "UNKNOWN"} | ${e.phase ?? "NO_PHASE"} | ${e.time_band ?? "NO_TIME"} | ${e.domain ?? "NO_DOMAIN"} | ${e.event_type ?? "NO_EVENT"} | ${e.status ?? "NO_STATUS"}`
    ),
...
export function runEvidenceLayer({
  ...
  const timingSummary = buildTimingSummary(rebuiltTimeline);
  const recognitionTimeline = buildRecognitionTimeline(rebuiltTimeline);
