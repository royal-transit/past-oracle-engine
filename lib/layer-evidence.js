...
function backfillRankedDomainsWithTimelineDates(ranked_domains, master_timeline) {
  ...
  return (ranked_domains || []).map((domain) => {
    ...
    const patchedPrimary =
      domain.primary_exact_event
        ? {
            ...domain.primary_exact_event,
            date_marker:
              normalizeYear(domain.primary_exact_event.date_marker) ??
              normalizeYear(domain.primary_exact_event.exact_year) ??
              (() => {
                const matched = patchedEvents.find(
                  (e) =>
                    e.event_type === domain.primary_exact_event.event_type &&
                    e.status === domain.primary_exact_event.status
                );
                return matched?.date_marker ?? null;
              })()
          }
        : null;
    ...
    const patchedAlternatives = (domain.alternative_exact_events || []).map((alt) => ({
      ...alt,
      date_marker:
        normalizeYear(alt.date_marker) ??
        normalizeYear(alt.exact_year) ??
        (() => {
          const matched = patchedEvents.find(
            (e) =>
