...
function exactTypeBoost(domainKey, event) {
  ...
  if (["BROKEN", "ACTIVE", "STABILISING", "EXECUTED", "BUILDING", "PARTIAL", "NOT_EXECUTED", "PENDING"].includes(status)) {
    boost += 2.5;
  }
...
function summarizeForeign(ledger, domainsByKey) {
  ...
  else foreignProcessStatus = getLastByStatuses(ledger, ["EXECUTED", "PARTIAL", "NOT_EXECUTED"])?.status || null;
...
function summarizeHealth(ledger, domainsByKey) {
  ...
  else healthStatus = getLastByStatuses(ledger, ["EXECUTED", "PARTIAL", "NOT_EXECUTED"])?.status || "UNKNOWN";

  const latest = getLastByStatuses(ledger, ["EXECUTED", "PARTIAL", "NOT_EXECUTED"]);
...
function summarizeMoney(ledger, domainsByKey) {
  ...
  else moneyStatus = getLastByStatuses(ledger, ["EXECUTED", "PARTIAL", "NOT_EXECUTED"])?.status || "UNKNOWN";
...
function summarizeConflict(ledger, domainsByKey) {
  ...
  else conflictStatus = getLastByStatuses(ledger, ["EXECUTED", "PARTIAL", "NOT_EXECUTED"])?.status || "UNKNOWN";
...
function rebuildTimelineFromPatchedDomains(ranked_domains) {
  ...
  return out
    .filter((x) => x.importance === "MAJOR" || x.status === "EXECUTED" || x.status === "PARTIAL")
    .sort((a, b) => {
