...
// Purpose:
// - flatten summary না করে domain-ledger driven evidence output
// - marriage/divorce/remarriage correct merge
// - education vs job/business reality-safe summary
// - foreign process vs executed separation
// - health / money / property / family / enemy domain-aware summary
...
function exactTypeBoost(domainKey, event) {
  ...
  if (["BROKEN", "ACTIVE", "STABILISING", "EXECUTED", "BUILDING", "PENDING"].includes(status)) {
    boost += 2.5;
  }
  ...
  if (["HEALTH", "MENTAL"].includes(domainKey)) {
    ...
    if (
      ["strain", "imbalance", "recovery", "stress", "fear", "body"]
  ...
  if (["MONEY", "DEBT", "GAIN", "LOSS"].includes(domainKey)) {
    if (
      ["money", "debt", "gain", "loss", "income", "cash", "liquidity", "pressure"]
        .some((x) => type.includes(x))
    ) boost += 4;

    if (
      ["money", "pressure", "flow", "compression", "gain", "loss"]
        .some((x) => trigger.includes(x))
    ) boost += 3.2;
  }
  ...
  if (["LEGAL", "DOCUMENT", "IMMIGRATION", "VISA"].includes(domainKey)) {
    if (
      ["legal", "document", "approval", "authority", "paperwork", "visa", "immigration"]
        .some((x) => type.includes(x))
    ) boost += 4;

    if (
      ["approval", "authority", "paperwork", "clearance", "motion"]
        .some((x) => trigger.includes(x))
    ) boost += 3.2;
  }
  ...
  if (["MARRIAGE", "DIVORCE", "LOVE", "PARTNERSHIP", "MULTIPLE_MARRIAGE"].includes(domainKey)) {
    ...
    if (
      ["union", "break", "rupture", "continuation", "bond", "attachment"]
        .some((x) => trigger.includes(x))
    ) boost += 3.2;
  }
...
function getDomainEvents(ranked_domains, key) {
  return (ranked_domains || []).find((d) => d.domain_key === key)?.events || [];
}
...
function summarizeRelationship(ledger) {
  ...
  const executedMarriageCount = countByTypesAndStatuses(
    ledger,
    marriageLikeTypes,
    ["EXECUTED", "ACTIVE", "STABILISING"]
  );

  const divorceCount = countByTypesAndStatuses(
    ledger,
    divorceLikeTypes,
    ["EXECUTED", "BROKEN", "ACTIVE"]
  );
  ...
  if (lastMarriage && !lastDivorce) {
    currentStatus = normalizeStatus(lastMarriage.status || "EXECUTED");
    ...
    currentStatus = my > dy
      ? normalizeStatus(lastMarriage.status || "EXECUTED")
      : "BROKEN";
  }

  return {
    relationship_ledger: ledger,
    marriage_count: executedMarriageCount,
    broken_marriage_count: divorceCount,
...
function summarizeWork(ledger) {
  const educationActive = ledger.some(
    (e) => normalizeType(e.event_type).includes("study") && ["ACTIVE", "EXECUTED", "STABILISING"].includes(normalizeStatus(e.status))
  );

  const jobActive = ledger.some(
    (e) => normalizeType(e.event_type).includes("job") && ["ACTIVE", "EXECUTED"].includes(normalizeStatus(e.status))
  );

  const businessActive = ledger.some(
    (e) => normalizeType(e.event_type).includes("business") && ["ACTIVE", "EXECUTED"].includes(normalizeStatus(e.status))
  );
  ...
  else if (jobActive) dominantMode = "JOB";
...
function summarizeForeign(ledger) {
  const entryEvent = getLastByTypes(ledger, ["foreign entry event"]);
  const settlementEvent = getLastByTypes(ledger, ["settlement event"]);
  const processEvent = getLastByTypes(ledger, ["foreign process / movement pattern", "visa / approval process", "document / paperwork phase"]);

  return {
    foreign_ledger: ledger,
    foreign_shift_count: ledger.filter((e) => normalizeType(e.event_type).includes("foreign")).length,
    ...
    foreign_process_status: processEvent ? normalizeStatus(processEvent.status) : null
  };
}

function summarizeHealth(ledger) {
  const active = getLastByStatuses(ledger, ["ACTIVE", "RESIDUAL", "BUILDING", "EXECUTED"]);
  return {
    health_ledger: ledger,
    health_status: active ? normalizeStatus(active.status) : "UNKNOWN",
    latest_health_event_type: active?.event_type || null
  };
}

function summarizeMoney(ledger) {
  const latest = ledger.length ? [...ledger].sort(byYearThenNumber).pop() : null;
  return {
    money_ledger: ledger,
    money_status: latest ? normalizeStatus(latest.status) : "UNKNOWN",
    latest_money_event_type: latest?.event_type || null
  };
}

function summarizeConflict(ledger) {
  const latest = ledger.length ? [...ledger].sort(byYearThenNumber).pop() : null;
  return {
    conflict_ledger: ledger,
    conflict_status: latest ? normalizeStatus(latest.status) : "UNKNOWN",
...
function buildForensicVerdict(question_profile, event_summary, carryover, validation_block) {
  ...
  const relationship_note =
    event_summary.relationship.current_marriage_status !== "UNKNOWN"
      ? ` Relationship ledger shows ${event_summary.relationship.marriage_count} executed marriage event(s), ${event_summary.relationship.broken_marriage_count} break event(s), current status ${event_summary.relationship.current_marriage_status}.`
      : "";

  const work_note =
    event_summary.work.dominant_work_mode !== "UNKNOWN"
      ? ` Work-study ledger resolves current dominant mode as ${event_summary.work.dominant_work_mode}.`
      : "";

  const foreign_note =
    event_summary.foreign.foreign_entry_year || event_summary.foreign.foreign_process_status
      ? ` Foreign ledger shows entry year ${event_summary.foreign.foreign_entry_year ?? "UNKNOWN"} and process status ${event_summary.foreign.foreign_process_status ?? "UNKNOWN"}.`
      : "";

  return {
    forensic_direction: forensic_direction + relationship_note + work_note + foreign_note,
    validation_state
  };
}
...
function buildLokkothaSummary(question_profile, event_summary, carryover) {
  return {
    text:
      `${String(question_profile.primary_domain || "GENERAL").toLowerCase()}-এর হিসাব সোজা লাইনে মেটে না; ` +
...
function buildProjectPasteBlock({
  ...
  return [
    ...
    "Timeline:",
    ...master_timeline.slice(0, 20).map(
      (e) =>
        `${e.domain} | #${e.event_number} | ${e.event_type} | ${e.status} | ${e.date_marker ?? "UNDATED"} | ${e.month_or_phase ?? "NO_PHASE"}`
    ),
...
export function runEvidenceLayer({
  ...
  const patched_ranked_domains = patchAndRebuildRankedDomains(ranked_domains, master_timeline);

  const relationshipLedger = buildRelationshipLedger(patched_ranked_domains);
  const workLedger = buildWorkLedger(patched_ranked_domains);
  const foreignLedger = buildForeignLedger(patched_ranked_domains);
  const healthLedger = buildHealthLedger(patched_ranked_domains);
  const moneyLedger = buildMoneyLedger(patched_ranked_domains);
  const conflictLedger = buildConflictLedger(patched_ranked_domains);

  const relationshipSummary = summarizeRelationship(relationshipLedger);
  const workSummary = summarizeWork(workLedger);
  const foreignSummary = summarizeForeign(foreignLedger);
  const healthSummary = summarizeHealth(healthLedger);
  const moneySummary = summarizeMoney(moneyLedger);
  const conflictSummary = summarizeConflict(conflictLedger);
