...
// FULL REPLACEMENT — LIGHT BUT REAL-LIFE DOMAIN CORRECTOR
  ...
  return (exactDomainSummary || []).map((item) => {
    ...
    if (out.domain_key === "SETTLEMENT") {
      ...
      if (hasRefused) {
        ...
        out.primary_exact_event.date_marker = facts?.settlement_refusal_year_claim || out.primary_exact_event.date_marker;
      }
      ...
      if (hasAppeal) {
        ...
        out.primary_exact_event.date_marker = facts?.appeal_year_claim || out.primary_exact_event.date_marker;
      } else if (hasApplied && !hasGranted) {
        out.primary_exact_event = partial(
          out.primary_exact_event,
          "application submitted but not completed"
        );
        out.primary_exact_event.event_type = "application in process";
        out.primary_exact_event.date_marker = facts?.settlement_applied_year_claim || out.primary_exact_event.date_marker;
      } else if (settlementTruth === "ongoing") {
        out.primary_exact_event = partial(
          out.primary_exact_event,
