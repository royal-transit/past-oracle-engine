...
// FULL REPLACEMENT — FACT ANCHOR MERGER V2
// Fix:
// - No domain gets EXECUTED unless that exact domain has a matching fact anchor
// - Love / partnership / divorce / settlement contamination blocked
// - Foreign fact mismatch becomes FACT_CONFLICT, not false EXECUTED

const VERSION = "FACT_ANCHOR_MERGER_V2";
...
function factYearForDomain(domainKey, facts = {}) {
  ...
  if (k === "EDUCATION") return facts.study_start_year_claim || null;
  if (k === "LOVE") return facts.love_year_claim || null;
  if (k === "MARRIAGE") return facts.marriage_year_claim || null;
  if (k === "DIVORCE") return facts.divorce_year_claim || facts.broken_marriage_year_claim || null;
  if (k === "PARTNERSHIP") return facts.partnership_year_claim || null;

  if (["FOREIGN", "IMMIGRATION"].includes(k)) {
    return facts.foreign_entry_year_claim || facts.student_visa_entry_year_claim || null;
  }

  if (k === "VISA") {
    return facts.visa_year_claim || facts.student_visa_entry_year_claim || facts.foreign_entry_year_claim || null;
  }

  if (k === "SETTLEMENT") {
    return facts.settlement_year_claim || facts.settlement_applied_year_claim || facts.route_shift_year_claim || null;
  }

  if (["LEGAL", "DOCUMENT"].includes(k)) {
    return facts.appeal_year_claim || facts.settlement_refusal_year_claim || facts.settlement_applied_year_claim || null;
  }

  if (["JOB", "CAREER"].includes(k)) return facts.job_start_year_claim || null;
  if (k === "BUSINESS") return facts.business_start_year_claim || null;
  if (["PROPERTY", "HOME", "VEHICLE"].includes(k)) return facts.property_year_claim || null;
  if (["MONEY", "DEBT"].includes(k)) return facts.debt_year_claim || null;
  if (k === "MENTAL") return facts.mental_year_claim || null;
  if (k === "HEALTH") return facts.health_year_claim || null;
...
function classifyMerge({ domain, event, factYear, facts }) {
  ...
  const candidateWindow = event.likely_year_window || event.broad_year_window || null;
  ...
  if (!year && countMatch) {
    return {
      ...
      note: "Fact count supports this domain only; no exact year provided."
    };
  }

  if (!year) {
    return {
      merge_status: "NO_DOMAIN_FACT_ANCHOR",
      exact_year: null,
      evidence_type: event.evidence_type,
      status: event.status,
      confidence: event.confidence_label || "ASTRO_FORENSIC",
      note: "No fact anchor for this domain; preserving astro state."
    };
  }
  ...
  if (candidateWindow && yearInsideWindow(year, candidateWindow)) {
    return {
      ...
      note: "Fact year matches this domain candidate window."
    };
  }
  ...
  if (primaryYear && Math.abs(year - primaryYear) <= 1) {
    return {
      ...
      status: "EXECUTED",
      confidence: "HIGH_FACT_ANCHORED",
      note: "Fact year matches this domain prime year."
    };
  }
  ...
  if (secondaryYears.includes(year)) {
    return {
      ...
      note: "Fact year matches this domain secondary year."
    };
  }
  ...
  return {
    ...
    note: "Domain fact exists but does not match astro/timeline candidate."
  };
}
...
function mergeEvent(domain, event, facts) {
  ...
  if (merge.merge_status === "NO_DOMAIN_FACT_ANCHOR") {
...
export function runFactAnchorMerger({ ranked_domains = [], facts = {} }) {
  const merged_domains = safeArray(ranked_domains).map((domain) => {
    const events = safeArray(domain.events).map((event) => mergeEvent(domain, event, facts));
