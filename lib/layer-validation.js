...
// UNIVERSAL CLIENT-SAFE
// PAST MODE: EXECUTED FACT IS NOT CONFLICT
// FUTURE/QUERY MODE: CONFLICT RULE STILL AVAILABLE
// KP CUSP FIRST → DOMAIN-SCOPED HOUSE-HIT FALLBACK
...
function isPastQuestion(question) {
  const q = normalizeText(question);
  return hasQuestionWord(q, ["past", "past history", "history", "previous", "before", "age", "happened"]);
}

function findDomain(domains, key) {
  return (domains || []).find((d) => up(d?.domain_key) === up(key)) || null;
}
...
function buildSectorState(finalizedDomains) {
  return {
    ...
    debt: finalStateOf(finalizedDomains, "DEBT"),
    partnership: finalStateOf(finalizedDomains, "PARTNERSHIP")
  };
}
...
function detectQuestionIntent(question) {
  ...
  return {
    ...
    asks_foreign: hasQuestionWord(q, ["foreign", "abroad", "uk", "visa", "immigration", "bidesh", "ilr"]),
    asks_settlement: hasQuestionWord(q, ["settlement", "permanent", "base", "ilr", "citizenship"]),
    ...
    asks_legal: hasQuestionWord(q, ["legal", "court", "authority", "case", "document", "appeal", "paperwork"]),
...
function eventDomainFromInput(eventInput = {}) {
  ...
  if (hasQuestionWord(text, ["foreign", "visa", "uk", "abroad", "immigration", "travel", "bidesh", "ilr", "settlement"]))
    return "FOREIGN";

  if (hasQuestionWord(text, ["court", "legal", "authority", "case", "document", "fine", "appeal", "paperwork"]))
    return "LEGAL";
...
function allowedDomainsForEventDomain(domain) {
  const map = {
    FOREIGN: ["IMMIGRATION", "VISA", "FOREIGN", "SETTLEMENT", "DOCUMENT", "LEGAL"],
    BUSINESS: ["BUSINESS", "CAREER", "JOB", "MONEY", "DOCUMENT", "LEGAL", "SUCCESS"],
    MONEY: ["MONEY", "DEBT", "PROPERTY", "BUSINESS", "JOB", "GAIN", "LOSS"],
    JOB: ["JOB", "CAREER", "BUSINESS", "MONEY", "LEGAL", "DOCUMENT"],
    RELATIONSHIP: ["MARRIAGE", "LOVE", "PARTNERSHIP", "DIVORCE", "FAMILY", "CHILDREN"],
    COMMUNICATION: ["COMMUNICATION", "DOCUMENT", "NETWORK", "LEGAL", "BUSINESS"],
    LEGAL: ["LEGAL", "DOCUMENT", "FOREIGN", "SETTLEMENT", "JOB"],
...
function collectHouseHits(domainResults, requiredCusps, allowedDomainKeys = []) {
  const hits = [];
  const required = new Set(requiredCusps.map(String));
  const allowed = new Set((allowedDomainKeys || []).map(up));
  const useDomainFilter = allowed.size > 0;

  for (const domain of domainResults || []) {
    const domainKey = up(domain?.domain_key);

    if (useDomainFilter && !allowed.has(domainKey)) continue;
...
export function kpPastValidation(snapshot = {}, eventInput = {}) {
  ...
  const allowedDomainKeys =
    eventInput.linked_domain_expansion ||
    snapshot.linked_domain_expansion ||
    allowedDomainsForEventDomain(domain);
  ...
  const houseFallbackHits = collectHouseHits(domainResults, requiredCusps, allowedDomainKeys);
  ...
  if (!activatedCusps.length && houseFallbackHits.length) {
    ...
    reasons.push(`Domain-scoped house-hit fallback active: ${fallbackHouses.join(",")}`);
  }
  ...
  return {
    ...
    allowed_domain_keys: allowedDomainKeys,
...
export function runValidationLayer({
  ...
  const pastMode = isPastQuestion(question);
  ...
  const results = [
    ...
    ruleValidateSector({ sector: "EDUCATION", asks: intent.asks_education, finalState: state.education, pastMode }),
    ruleValidateSector({ sector: "JOB", asks: intent.asks_job, finalState: state.job, pastMode }),
    ruleValidateSector({ sector: "BUSINESS", asks: intent.asks_business, finalState: state.business, pastMode }),
    ruleValidateSector({ sector: "MONEY", asks: intent.asks_money, finalState: state.money, pastMode }),
    ruleValidateSector({ sector: "PROPERTY", asks: intent.asks_property, finalState: state.property, pastMode }),
    ruleValidateSector({ sector: "HEALTH", asks: intent.asks_health, finalState: state.health, pastMode }),
    ruleValidateSector({ sector: "MENTAL", asks: intent.asks_mental, finalState: state.mental, pastMode }),
    ruleValidateSector({ sector: "LEGAL", asks: intent.asks_legal, finalState: state.legal, pastMode }),
    ruleValidateSector({ sector: "FAMILY", asks: intent.asks_family, finalState: state.family, pastMode }),
    ruleValidateSector({ sector: "CHILDREN", asks: intent.asks_children, finalState: state.children, pastMode })
  ];
  ...
  return {
    ...
    kp_past_validation:
      validationSnapshot && eventInput
        ? kpPastValidation(validationSnapshot, {
            ...eventInput,
            question,
            linked_domain_expansion:
              eventInput?.linked_domain_expansion ||
              validationSnapshot?.linked_domain_expansion ||
              allowedDomainsForEventDomain(eventDomainFromInput({ ...eventInput, question }))
          })
        : null
  };
}