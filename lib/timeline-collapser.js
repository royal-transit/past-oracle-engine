...
// FULL REPLACEMENT — ELITE TIMELINE COLLAPSER V2
// Purpose:
// - NAME_ONLY = age window only
// - NAME+DOB = likely year window only
// - FULL_BIRTH = prime year candidate, but NOT execution seal
// - DIRECT_FACT only can support executed certainty
// - Prevent broad full-birth timing

const VERSION = "ELITE_TIMELINE_COLLAPSER_V2";
...
const DOMAIN_AGE_WINDOWS = {
  ...
  MONEY:[22,44], DEBT:[24,44], PROPERTY:[28,48], VEHICLE:[22,40],
  LOVE:[20,32], MARRIAGE:[24,34], DIVORCE:[28,42],
  MULTIPLE_MARRIAGE:[28,44], PARTNERSHIP:[24,40], CHILDREN:[27,40],
  FOREIGN:[20,29], IMMIGRATION:[20,31], VISA:[19,30], SETTLEMENT:[28,40],
  DOCUMENT:[20,42], HEALTH:[24,46], MENTAL:[18,42],
  LEGAL:[26,44], ENEMY:[24,44], BLOCKAGE:[22,42],
  SUCCESS:[28,46], SPIRITUAL:[28,50], RELIGION:[28,50]
};
...
function confidenceLabel({ modeFlags, event, collapse }) {
  ...
  const total = num(collapse?.scoring?.total_timeline_score);
...
function collapseEvent({ domain, event, birth_context, subject_context, evidence_packet }) {
  ...
  if (modeFlags.ageWindowOnly || !birthYear) {
    ...
    e.month_or_phase = `likely age window ${e.likely_age_window[0]}-${e.likely_age_window[1]}`;
    e.precision_label = "AGE_WINDOW_ONLY";
    e.confidence_label = "NAME_PATTERN_ONLY";
  ...
  if (modeFlags.yearWindowAllowed && !modeFlags.primeYearAllowed) {
    ...
    e.month_or_phase = `likely year window ${broad[0]}-${broad[1]}`;
