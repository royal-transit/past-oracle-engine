...
// FULL LIFE DOMAIN DELIVERY MAPPER
...
export function buildTruthLines(domainMap = {}) {
  ...
  if (domainMap.relationship) {
    ...
    if (r.multiple_marriage === "occurred") lines.push("একাধিক সম্পর্ক / বিয়ের pattern জীবনে কাজ করেছে");
  }
  ...
  if (domainMap.work) {
    ...
    if (w.job === "occurred") lines.push("কাজ বা চাকরির line বাস্তবে খুলেছে");
    if (w.job === "ongoing") lines.push("কাজের line চলছে, কিন্তু পুরো settle হয়নি");
  ...
  // CONFLICT
  if (domainMap.conflict) {
    const c = domainMap.conflict;

    if (c.legal === "occurred") lines.push("authority / legal pressure বাস্তবে এসেছে");
  ...
  // FAMILY
  if (domainMap.family) {
    const f = domainMap.family;

    if (f.family === "occurred") lines.push("পরিবার বা দায়িত্বের line বাস্তবে এসেছে");
    if (f.family === "ongoing") lines.push("পরিবারের দায় এখনো কাঁধে আছে");

    if (f.children === "occurred") lines.push("সন্তান / continuity line বাস্তবে ঘটেছে");
    if (f.children === "ongoing") lines.push("সন্তান / continuity line প্রক্রিয়ায় আছে");
  }

  // EDUCATION
  if (domainMap.education) {
