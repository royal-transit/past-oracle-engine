// lib/layer-astro.js
// FULL REPLACEMENT — UNIVERSAL PAST NANO CORE V8.1
// NAME_ONLY_STRONG_PATTERN_MODE FIXED
// Name-only creates likely timing windows.
// Name-only never confirms execution.
...
const STANDARD_PATTERN_THRESHOLD = 5;
...
function strengthLabel(score) {
  return score >= 3.2 && score < 5 ? "strong_name_pattern" :
    score >= 8 ? "strong" :
    score >= 5 ? "moderate" :
    score >= 2.2 ? "light_but_usable" :
    "weak";
}
...
function buildPatternEvent(domainKey, score, modeFlags, birth_context) {
  ...
  return makeEvent({
    event_family: modeFlags.isNameOnly || modeFlags.isNameContext ? "NAME_PATTERN_LEDGER" : "ASTRO_PATTERN_LEDGER",
    ...
    status: "PATTERN_ONLY",
    importance: score >= NAME_ONLY_MAJOR_THRESHOLD ? "MAJOR" : "MINOR",
    trigger_phase: modeFlags.isNameOnly || modeFlags.isNameContext
      ? "strong name-pattern timing window"
      : "astro pattern timing window",
    carryover_to_present: "NO",
    who_involved: "self / domain-linked people",
    impact_summary: "strong probability window visible; execution is not confirmed",
    evidence_type: modeFlags.isNameOnly || modeFlags.isNameContext ? "NAME_PATTERN" : "LIKELY_HISTORY",
...
function buildFactOrAstroEvents(domainKey, hints, score, modeFlags, birth_context) {
  ...
  const patternThreshold = modeFlags.isNameOnly || modeFlags.isNameContext
    ? NAME_ONLY_EVENT_THRESHOLD
    : STANDARD_PATTERN_THRESHOLD;
  ...
  if (["FOREIGN", "IMMIGRATION", "VISA", "SETTLEMENT"].includes(domainKey)) {
    ...
    if (score >= patternThreshold) return [buildPatternEvent(domainKey, score, modeFlags, birth_context)];
    return [];
  }
...
export function runAstroLayer({ evidence_packet, facts, subject_context, birth_context }) {
  ...
  let domain_results = DOMAIN_REGISTRY.map(([domainKey, domainLabel, targetHouses]) => {
    ...
    return {
      ...
      active_event_count: events.filter((e) => ["ACTIVE", "STABILISING", "EXECUTED", "BUILDING", "PATTERN_ONLY"].includes(e.status)).length,
      events
    };
  });
