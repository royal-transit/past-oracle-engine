// lib/layer-astro.js
// UNIVERSAL FULL LIFE-SECTOR HARD-LOCK VERSION
// Goal:
// - all major life sectors use explicit event-family builders
// - no hard completion without support
// - no pseudo-year for hard sectors
// - process != executed
// - pattern != history
...
function buildHints(facts) {
  ...
  return {
    ...
    foreignExecuted: hasAny(raw, [
      "went abroad", "arrived uk", "came uk", "entered uk", "moved abroad",
      "foreign entry", "immigrated", "overseas move"
    ]),

    settlementMention: hasAny(raw, [
      "settlement", "settled", "permanent", "stable base", "residence", "base"
    ]),

    marriageMention: hasAny(raw, [
      "marriage", "married", "bea", "biye", "nikah", "wedding", "wife", "husband", "spouse"
    ]),
...
function calculateDomainBase(domainKey, targetHouses, evidence, hints) {
  ...
  if (domainKey === "CAREER" && (hints.hasJob || hints.hasBusiness)) rawScore += 2.2;

  if (domainKey === "MARRIAGE" && (hints.marriageMention || hints.marriage_count_claim != null)) rawScore += 2.8;
  if (domainKey === "DIVORCE" && (hints.divorceMention || hints.broken_marriage_claim != null)) rawScore += 2.6;
  if (domainKey === "MULTIPLE_MARRIAGE" && ((hints.marriage_count_claim ?? 0) >= 2 || hints.remarriageMention)) rawScore += 2.4;

  if (domainKey === "FOREIGN" && (hints.foreignIntent || hints.foreign_entry_year_claim != null)) rawScore += 2.2;
  if (domainKey === "SETTLEMENT" && (hints.settlementYearHint != null || hints.settlementMention)) rawScore += 2.3;
  if (domainKey === "IMMIGRATION" && hints.foreignIntent) rawScore += 2.0;
  if (domainKey === "VISA" && hints.foreignIntent) rawScore += 1.6;
  if (domainKey === "DOCUMENT" && hints.documentMention) rawScore += 1.6;

  if (domainKey === "HEALTH" && hints.healthMention) rawScore += 2.4;
  if (domainKey === "MENTAL" && hints.mentalMention) rawScore += 2.2;
  if (domainKey === "FAMILY" && hints.familyMention) rawScore += 1.8;
  ...
  if (domainKey === "LOVE" && hints.relationshipMention) rawScore += 1.8;
  if (domainKey === "CHILDREN" && hints.childrenMention) rawScore += 1.8;
  if (domainKey === "SPIRITUAL" && hints.spiritualMention) rawScore += 1.8;
  if (domainKey === "HOME" && hints.propertyMention) rawScore += 1.3;
...
function buildMarriageEvents(hints, score) {
  ...
  const total = hints.marriage_count_claim ?? (hints.remarriageMention ? 2 : hints.marriageMention ? 1 : 0);
  ...
  if (total > 0) {
    for (let i = 0; i < total; i += 1) {
      events.push(makeEvent({
        ...
        event_type: i === 0 ? "first marriage event" : i === 1 ? "second marriage event" : `marriage event #${i + 1}`,
        event_number: i + 1,
...
function buildMultipleMarriageEvents(hints) {
...
function buildLoveEvents(hints, score) {
  if (!hints.relationshipMention && score < 7) return [];
  return [
    makeEvent({
      event_family: "RELATIONSHIP_LEDGER",
...
function buildChildrenEvents(hints, score) {
  if (!hints.childrenMention && score < 7) return [];
  return [
    makeEvent({
...
function buildForeignEvents(hints, score) {
  const events = [];
  ...
  if (entryYear != null || hints.foreignExecuted) {
    ...
    events.push(makeEvent({
      ...
      carryover_to_present: "YES",
      who_involved: "self / authority",
      impact_summary: "foreign process visible, completion unconfirmed"
    }));
  }

  if (settlementYear != null || hints.settlementMention) {
    events.push(makeEvent({
      event_family: "FOREIGN_LEDGER",
      ...
      evidence_strength: settlementYear ? "strong" : strengthLabel(score),
      status: settlementYear ? "STABILISING" : "BUILDING",
      importance: "MAJOR",
      trigger_phase: "base consolidation",
      carryover_to_present: "YES",
      who_involved: "self / residence / authority",
      impact_summary: settlementYear ? "base formation / settlement" : "settlement tendency visible, completion unconfirmed"
    }));
  }
...
function buildEducationEvents(hints, score) {
  if (!hints.isStudent && score < 7) return [];
  return [
    makeEvent({
...
function buildHealthEvents(domainKey, hints, score) {
  if (!hints.healthMention && !hints.mentalMention && score < 7) return [];
  const type =
...
function buildFamilyEvents(hints, score) {
  if (!hints.familyMention && !hints.familyHealthMention && score < 7) return [];
  return [makeEvent({
    event_family: "FAMILY_LEDGER",
    event_type: hints.familyHealthMention ? "family health pressure phase" : "family responsibility phase",
    exact_year: hints.years[0] ?? null,
    month_or_phase: hints.years.length ? "year-anchored phase" : "phase only",
    evidence_strength: strengthLabel(score),
    status: hints.familyMention || hints.familyHealthMention ? "ACTIVE" : "RESIDUAL",
    importance: "MAJOR",
    trigger_phase: hints.familyHealthMention ? "family weakness / care phase" : "family involvement",
    carryover_to_present: "YES",
...
function buildLegalEvents(domainKey, hints, score) {
  if (!hints.legalMention && !hints.documentMention && score < 7) return [];
  const type =
...
function buildEnemyEvents(hints, score) {
  if (!hints.enemyMention && score < 7) return [];
  return [makeEvent({
...
function buildNetworkEvents(hints, score) {
  if (!hints.networkMention && score < 7) return [];
  return [makeEvent({
...
function buildSpiritualEvents(hints, score) {
  if (!hints.spiritualMention && score < 7) return [];
  return [makeEvent({
...
function inferEvents(domainKey, score, facts) {
  ...
  switch (domainKey) {
    ...
    case "MULTIPLE_MARRIAGE":
      return buildMultipleMarriageEvents(hints);
    case "LOVE":
    case "PARTNERSHIP":
    ...
    case "IMMIGRATION":
    case "VISA":
      return buildForeignEvents(hints, score);
    case "EDUCATION":
...
export function buildTimeline({ ranked_domains, birth_context }) {
  ...
  return out
    .filter((x) =>
      x.importance === "MAJOR" ||
      [
        "MARRIAGE", "DIVORCE", "FOREIGN", "SETTLEMENT", "IMMIGRATION", "EDUCATION",
        "JOB", "BUSINESS", "CAREER", "HEALTH", "MONEY", "PROPERTY", "LEGAL", "FAMILY"
