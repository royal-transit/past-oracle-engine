// lib/layer-astro.js
// FULL REPLACEMENT — UNIVERSAL PAST NANO CORE V8.6
// FULL_BIRTH timing compression active
// NAME_ONLY remains likely age window only
...
export function normalizeEvidence(packet = {}) {
  ...
  if (!Object.keys(housesByPlanet).length && asc?.sign) {
...
function patternEventType(domainKey) {
  const map = {
    LEGAL:"likely legal / authority-pressure window",
    DIVORCE:"likely separation / divorce-pressure window",
    MONEY:"likely money-liquidity pressure window",
    DOCUMENT:"likely paperwork / document-authority window",
    ...
    HEALTH:"likely health / stress window",
    FOREIGN:"likely foreign / distance movement window",
    EDUCATION:"likely study / qualification window"
  };
  return map[domainKey] || `likely ${String(domainKey || "domain").toLowerCase()} window`;
}
...
function compressYearWindow(domainKey, broadWindow, score, supportCount, base) {
  if (!Array.isArray(broadWindow)) return null;

  const [start, end] = broadWindow;
  const span = end - start;

  let width = 7;
  if (score >= 8.2 && supportCount >= 9) width = 4;
  else if (score >= 7.4 && supportCount >= 7) width = 5;
  else if (score >= 6.4 && supportCount >= 5) width = 6;

  const kpPull = Number(base.kp_weight || 0);
  const housePull = (base.houseHits || []).length;
  const aspectPull = (base.aspectHits || []).length;

  let anchorRatio = 0.5;
  anchorRatio += Math.min(0.18, kpPull * 0.035);
  anchorRatio += Math.min(0.12, housePull * 0.018);
  anchorRatio -= Math.min(0.08, aspectPull * 0.006);

  if (["IMMIGRATION","FOREIGN","VISA","EDUCATION"].includes(domainKey)) anchorRatio -= 0.18;
  if (["MARRIAGE","LOVE","PARTNERSHIP"].includes(domainKey)) anchorRatio -= 0.03;
  if (["DIVORCE","LEGAL","DOCUMENT","MONEY","BUSINESS","HEALTH"].includes(domainKey)) anchorRatio += 0.10;

  anchorRatio = Math.max(0.12, Math.min(0.82, anchorRatio));

  const anchor = Math.round(start + span * anchorRatio);
  const half = Math.floor(width / 2);

  let a = Math.max(start, anchor - half);
  let b = Math.min(end, a + width);

  if (b - a < width) a = Math.max(start, b - width);
...
function makeEvent(payload) {
  return {
    ...
    broad_year_window: payload.broad_year_window || null,
    timing_mode: payload.timing_mode || "PATTERN_WINDOW"
  };
}

function buildPatternEvent(domainKey, score, modeFlags, birth_context, base) {
  const aw = ageWindow(domainKey);
  const broad = modeFlags.isReducedDob || modeFlags.isFullBirth
    ? yearWindowFromAge(birth_context?.birth_datetime_iso, domainKey)
    : null;

  const compressed =
    modeFlags.isFullBirth
      ? compressYearWindow(domainKey, broad, score, base.supportCount, base)
      : broad;
  ...
  return makeEvent({
    ...
    month_or_phase: compressed
      ? `compressed year window ${compressed[0]}-${compressed[1]}`
      : `likely age window ${aw[0]}-${aw[1]}`,
    evidence_strength: strengthLabel(score, modeFlags),
    status: isName ? "LIKELY_WINDOW" : score >= 7.35 ? "VERY_HIGH_PROBABILITY" : "HIGH_PROBABILITY",
    importance: score >= majorThreshold(modeFlags) ? "MAJOR" : "MINOR",
    trigger_phase: isName ? "elite name-pattern locked window" : "full-birth astro compressed probability window",
    ...
    impact_summary: isName
      ...
      : "compressed full-birth probability window; execution not sealed without direct factual anchor",
    evidence_type: isName ? "NAME_PATTERN" : "LIKELY_HISTORY",
    likely_age_window: aw,
    likely_year_window: compressed,
    broad_year_window: broad,
    timing_mode: compressed ? "COMPRESSED_YEAR_WINDOW" : "LIKELY_AGE_WINDOW"
  });
}

function buildFactOrAstroEvents(domainKey, hints, score, modeFlags, birth_context, base) {
  ...
  return [buildPatternEvent(domainKey, score, modeFlags, birth_context, base)];
}

function applyModePolicy(domain_results, modeFlags) {
  return domain_results.map((d) => {
    ...
    const events = (d.events || []).map((e) => {
      ...
      return {
        ...e,
        exact_claim_locked: e.evidence_type !== "DIRECT_FACT",
        execution_claim_allowed: e.evidence_type === "DIRECT_FACT"
      };
    });
...
export function runAstroLayer({ evidence_packet, facts, subject_context, birth_context }) {
  ...
  let domain_results = DOMAIN_REGISTRY.map(([domainKey, domainLabel, targetHouses]) => {
    ...
    const events = buildFactOrAstroEvents(domainKey, hints, base.normalizedScore, modeFlags, birth_context, base);
    ...
    return {
      ...
      active_event_count: events.filter((e) =>
        ["ACTIVE","STABILISING","EXECUTED","BUILDING","PATTERN_ONLY","LIKELY_WINDOW","HIGH_PROBABILITY","VERY_HIGH_PROBABILITY"].includes(e.status)
      ).length,
      events
    };
  });

  domain_results = applyModePolicy(domain_results, modeFlags);
...
export function buildTimeline({ ranked_domains, birth_context, subject_context }) {
  ...
  for (const d of ranked_domains || []) {
    for (const e of d.events || []) {
      out.push({
        ...
        likely_age_window: e.likely_age_window || ageWindow(d.domain_key),
        likely_year_window: modeFlags.isReducedDob || modeFlags.isFullBirth ? e.likely_year_window : null,
  ...
  return out.sort((a, b) => {
    const aa = a.date_marker ?? a.likely_year_window?.[0] ?? a.likely_age_window?.[0] ?? 99999;
    const bb = b.date_marker ?? b.likely_year_window?.[0] ?? b.likely_age_window?.[0] ?? 99999;
    return aa - bb;
  });
}