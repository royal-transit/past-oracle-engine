// lib/layer-astro.js
// FULL REPLACEMENT — UNIVERSAL PAST NANO CORE V8.3
// Sector compression + Full-birth sharper scoring
// NAME_ONLY = elite probability windows, no executed claim
// FULL_BIRTH = KP + Dasha + House + Aspect weighted forensic mode
...
const FULL_BIRTH_EVENT_THRESHOLD = 6.4;
const FULL_BIRTH_MAJOR_THRESHOLD = 8.2;
const REDUCED_DOB_EVENT_THRESHOLD = 5.8;
const STANDARD_PATTERN_THRESHOLD = 5.4;
...
function strengthLabel(score, modeFlags) {
  ...
  if (score >= 5.2) return "light_but_usable";
  return "weak";
}
...
function buildKpWeight(domainKey, targetHouses, evidence, modeFlags) {
  ...
  return {
    kp_weight: round2(Math.min(weight, 4.2)),
    kp_reasons: reasons
  };
}
...
function buildDashaWeight(domainKey, evidence, modeFlags) {
  ...
  lords.forEach((lord, idx) => {
    ...
    const add = idx === 0 ? 1.45 : idx === 1 ? 0.95 : 0.45;
    weight += add;
    reasons.push(`Dasha ${lord}`);
  });

  return {
    dasha_weight: round2(Math.min(weight, 3.0)),
...
function calculateDomainBase(domainKey, targetHouses, evidence, hints, modeFlags, nameHints) {
  ...
  if (!modeFlags.isNameOnly && !modeFlags.isNameContext) {
    for (const [planetName, houseNum] of Object.entries(evidence.housesByPlanet || {})) {
      if (targetHouses.includes(Number(houseNum))) {
        houseHits.push({ planet: planetName, house: Number(houseNum) });

        let weight = 1.15;
        if (BENEFICS.includes(planetName)) weight += 0.7;
        if (MALEFICS.includes(planetName)) weight += 0.9;

        if (domainPlanetLords(domainKey).includes(planetName)) weight += 1.15;
    ...
    for (const asp of evidence.aspects || []) {
      ...
      if (targetHouses.includes(h1) || targetHouses.includes(h2)) {
        ...
        rawScore += lordHit ? 0.9 : 0.45;
      }
    }
  }

  rawScore += factBoost(domainKey, hints);

  if (modeFlags.isNameOnly || modeFlags.isNameContext) {
    rawScore += nameOnlyBoost(domainKey, nameHints);
  }

  if (modeFlags.isReducedDob) rawScore += 0.65;
  if (modeFlags.isFullBirth) rawScore += 0.95;
  ...
  const supportCount =
    ...
    (factBoost(domainKey, hints) > 0 ? 1 : 0);

  let compression = 0;

  if (modeFlags.isFullBirth) {
    if (supportCount === 0) compression = -2.4;
    else if (supportCount === 1) compression = -1.2;
    else if (supportCount >= 3) compression = 0.75;
  }

  if (modeFlags.isReducedDob) {
    if (supportCount === 0) compression = -1.4;
    else if (supportCount >= 2) compression = 0.45;
  }

  const normalizedScore = round2(Math.max(0, Math.min(10, rawScore + targetHouses.length * 0.12 + compression)));
  ...
  return {
    ...
    density: normalizedScore >= 8 ? "HIGH" : normalizedScore >= 6 ? "MED" : "LOW",
    residualImpact: normalizedScore >= 8 ? "HIGH" : normalizedScore >= 6 ? "MED" : "LOW"
  };
}
...
function majorThreshold(modeFlags) {
  ...
  if (modeFlags.isReducedDob) return 7.2;
  return 7.0;
}
...
function buildFactOrAstroEvents(domainKey, hints, score, modeFlags, birth_context) {
  ...
  const threshold = eventThreshold(modeFlags);
  ...
  if (["FOREIGN", "IMMIGRATION", "VISA", "SETTLEMENT"].includes(domainKey)) {
    ...
    if (isDirect && modeFlags.execution_claim_allowed) {
      return [makeEvent({
    ...
    if (score >= threshold) return [buildPatternEvent(domainKey, score, modeFlags, birth_context)];
    return [];
  }

  if (["MARRIAGE", "DIVORCE", "MULTIPLE_MARRIAGE", "LOVE", "PARTNERSHIP", "CHILDREN"].includes(domainKey)) {
    const directRel =
      hints.marriageMention ||
      hints.marriageDoneByRole ||
      hints.divorceMention ||
      hints.relationshipMention ||
      hints.childrenMention;

    if (directRel && modeFlags.execution_claim_allowed) {
      return [makeEvent({
  ...
  return domain_results.map((d) => {
    ...
    const events = (d.events || []).map((e) => {
      if (isPatternMode) {
        return {
          ...e,
          status: d.normalized_score >= NAME_ONLY_MAJOR_THRESHOLD ? "LIKELY_WINDOW" : "PATTERN_ONLY",
      ...
      if (modeFlags.isReducedDob && ["NAME_PATTERN", "LIKELY_HISTORY", "PATTERN_ONLY"].includes(e.evidence_type)) {
        return {
          ...e,
...
function suppressWeakDomains(domain_results, modeFlags) {
  if (modeFlags.isNameOnly || modeFlags.isNameContext) {
    return domain_results;
  }

  const threshold = modeFlags.isFullBirth ? FULL_BIRTH_EVENT_THRESHOLD : REDUCED_DOB_EVENT_THRESHOLD;

  return domain_results.map((d) => {
    const hasEvents = (d.events || []).length > 0;
    const hasDirect = Number(d.direct_evidence_count || 0) > 0;
    const hasStrongSupport =
      Number(d.kp_weight || 0) > 0 ||
      Number(d.dasha_weight || 0) > 0 ||
      (d.house_hits || []).length > 0 ||
      (d.aspect_hits || []).length > 0;

    if (hasEvents || hasDirect) return d;

    if (d.normalized_score < threshold && !hasStrongSupport) {
      return {
        ...d,
        normalized_score: round2(Math.max(0, d.normalized_score - 1.25)),
        density: "LOW",
        residual_impact: "LOW",
        final_state: "UNKNOWN",
        suppressed_by_sector_compression: true
      };
    }
...
export function runAstroLayer({ evidence_packet, facts, subject_context, birth_context }) {
  ...
  domain_results = suppressWeakDomains(domain_results, modeFlags);
...
export function buildTimeline({ ranked_domains, birth_context, subject_context }) {
  ...
  ranked_domains.forEach((d) => {
    (d.events || []).forEach((e) => {
      ...
      if (modeFlags.isNameOnly || modeFlags.isNameContext) date_marker = null;
